...
job()
{
	...
	source log.sh
	local log_prefix="-- [${folderName} init script]: "
	...
	if [ ! -d $android_dir/app ]; then
		# Initial setup on a fresly created project.
		# After this code working out you can copy the created app folder to the
		# $android_dir directory and remove org.libsdl.app package from there.

		log_info "Initial setup the project for application '$app'"

		# Setup the app package
		local appdir="${app//"."/"/"}" 
		log_info "Create appdir '$appdir' at 'app/src/main/java'"
		local appdir_path="app/src/main/java/$appdir"
		if [ ! -d "$appdir_path" ]; then
			source $THIS_DIR/../automation_config.sh
			source "$scripts_dir/automation/templates/templates_config.sh"
			mkdir -p "$appdir_path"
			cp "$templates_dir/Java/SDL/MainActivity.java" "$appdir_path/"
		...
		log_info "Copy ready application files '$app' from '$android_dir/app' to '${PWD}'"
		cp -R "$android_dir/app" .
	fi
